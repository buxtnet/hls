...
# Đảm bảo Nginx có thể đọc thư mục webroot
chown -R nginx:nginx "$WEBROOT"
chmod -R 755 "$WEBROOT/.well-known/acme-challenge/"

# Tạo tệp thử thách để kiểm tra sau khi Nginx được cấu hình
...
# ---------- Khởi chạy ứng dụng với PM2 (Bắt buộc hoạt động trước khi Certbot chạy) ----------
if [ -d "$TARGET_DIR" ] && [ -f "${TARGET_DIR}/${ECOSYSTEM}" ]; then
  cd "$TARGET_DIR"
  npm install --omit=dev || true
  pm2 start "$ECOSYSTEM"
  pm2 save
  echo "Node.js application started on port 3000 via PM2."
fi

# ---------- Cấu hình Nginx HTTP cho MAINDOMAIN (Cần cho Webroot Challenge) ----------
...
# Tải lại Nginx để cấu hình port 80 có hiệu lực
nginx -t && systemctl reload nginx

# -------------------------------------------------------------
# ---------- BẮT ĐẦU PHẦN CERTBOT (Gần cuối script) ----------
# -------------------------------------------------------------
...
# Kiểm tra tệp thử thách có thể truy cập được không (Yêu cầu Nginx đã reload ở trên)
echo "Testing Certbot challenge file access..."
if ! curl -s "http://$MAINDOMAIN/.well-known/acme-challenge/test" | grep -q "test"; then
  echo "Error: Unable to serve the challenge file at http://${MAINDOMAIN}/.well-known/acme-challenge/test"
  echo "Please check your DNS A record and Nginx configuration."
  # Không thoát, vẫn cố gắng chạy Certbot
fi
echo "Challenge file is accessible via HTTP on ${MAINDOMAIN}."
...
    # Cần thiết cho việc gia hạn Certbot
    location ^~ /.well-known/acme-challenge/ {
        root ${WEBROOT};
        try_files \$uri =404;
    }

    # Redirect traffic from MAINDOMAIN (with SSL) to SECDOMAIN
    location / {
...
    listen 80;
    server_name ${MAINDOMAIN};
    # Redirect HTTP to HTTPS for MAINDOMAIN
...
    # Nếu không có cert, vẫn giữ cấu hình HTTP ban đầu (đã tạo ở trên)
    echo "No SSL certificate found for ${MAINDOMAIN}. Sticking to HTTP configuration for now."
fi
...
server {
    listen 80;
    listen 443 ssl http2; # Lắng nghe cả 80 và 443 cho Cloudflare Proxy

    server_name ${SECDOMAIN};

    # Cấu hình SSL cho Cloudflare Proxy (Self-signed hoặc Cloudflare Origin CA)
    # KHÔNG DÙNG LE, chỉ cần cho Origin. Tùy chọn: có thể dùng LE để bảo mật.
    # Để đơn giản, tôi sẽ giả định bạn dùng Cloudflare SSL/TLS Full/Strict mode
    # và không cần cert LE trên SECDOMAIN (vì nó là proxy).
    
    # Nếu bạn dùng Cloudflare Origin CA hoặc Certbot cho SECDOMAIN, hãy thêm các dòng này:
    # ssl_certificate /path/to/fullchain.pem;
    # ssl_certificate_key /path/to/privkey.pem;
    # ssl_protocols TLSv1.2 TLSv1.3;

    # Chặn truy cập trực tiếp bằng IP (Tùy chọn: Dùng $http_host thay cho $host để lấy host từ client)
    # if (\$http_host !~* ^(${SECDOMAIN}|${MAINDOMAIN})\$) {
    #     return 444;
    # }
...
    location ~ ^/([0-9]+)(/.*)?\$ {
        # Map /12345/something to /old_id/12345/something
...
    # Fallback (Cung cấp các tệp tĩnh hoặc proxy phần còn lại)
    location / {
        proxy_pass http://127.0.0.1:3000\$request_uri;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_set_header Host \$host;
...
echo "Notes:"
echo "- MAINDOMAIN is set up to issue a certificate and redirect all traffic to the SECDOMAIN using HTTPS."
echo "- SECDOMAIN is configured to listen on both 80/443, proxying to your Node app on port 3000."
echo "- Ensure your SECDOMAIN is set to 'Proxied' (Orange Cloud) in Cloudflare DNS for this config to work well."
echo "- Ensure your MAINDOMAIN is set to 'DNS-only' (Grey Cloud) in Cloudflare for Certbot execution."
echo "- If Certbot fails, check that MAINDOMAIN resolves to this server's IP address."
